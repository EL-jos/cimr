...
@section('main-content')
    <div class="card card-warning">
        ...
        <div class="card-body">
            <form action="{{ $role->exists ? route('role.update', $role) : route('role.store') }}" method="post" enctype="multipart/form-data">
                @csrf
                @method($role->exists ? 'PUT' : 'POST')
                <div class="row">
                    <div class="col-sm-12">
                        <div class="form-group">
                            <input type="text" class="form-control is-valid" name="name" id="name" placeholder="Nom ..." value="{{ $role->exists ? $role->name : old('name') }}">
                        </div>
                    </div>
                </div>
