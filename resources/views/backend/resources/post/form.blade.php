...
@section('main-content')
    <div class="card card-warning">
        ...
        <div class="card-body">
            <form action="{{ $post->exists ? route('post.update', $post) : route('post.store') }}" method="post" enctype="multipart/form-data">
                @csrf
                @method($post->exists ? 'PUT' : 'POST')
                <div class="row">
                    <div class="col-sm-12">
                        <div class="form-group">
                            <input type="text" class="form-control is-valid" name="name" id="name" placeholder="Nom ..." value="{{ $post->exists ? $post->name : old('name') }}">
                        </div>
                    </div>
                </div>
