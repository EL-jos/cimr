...
@section('main-content')
    <div class="card card-warning">
        ...
        <div class="card-body">
            <form action="{{ $type->exists ? route('type.update', $type) : route('type.store') }}" method="post" enctype="multipart/form-data">
                @csrf
                @method($type->exists ? 'PUT' : 'POST')
                <div class="row">
                    <div class="col-sm-6">
                        <div class="form-group">
                            <input type="text" class="form-control is-valid" name="name" id="name" placeholder="Nom ..." value="{{ $type->exists ? $type->name : old('name') }}">
                        </div>
                    </div>
                    <div class="col-sm-6">
                        <div class="form-group">
                            <input type="number" class="form-control is-valid" name="nb_days" id="nb_days" placeholder="Nombre de jours de congé ..." value="{{ $type->exists ? $type->nb_days : old('nb_days') }}">
                        </div>
                    </div>
                </div>
